...
- **Zod generator**: `zod-prisma-types` runs alongside `prisma-client` and emits Zod schemas to `src/generated/zod/`. Import model schemas from there instead of hand-writing Zod for DB rows. Hand-written DTO schemas in `src/lib/schemas/` extend or `pick` from these.
- **Never import the `@/generated/zod` barrel in client-bundled code.** `src/generated/zod/index.ts` re-exports every `modelSchema/` and `inputTypeSchemas/` file, and several transitively import `@prisma/client/runtime/client.mjs`. If any **client component** imports so much as an enum schema from the barrel (`import { X } from "@/generated/zod"`), Turbopack tries to bundle the Prisma runtime for the browser and fails with `the chunking context (unknown) does not support external modules (request: node:module)`. Import the specific file instead: `import { UserSchema } from "@/generated/zod/modelSchema/UserSchema"` or `@/generated/zod/inputTypeSchemas/<X>Schema`. The barrel is safe in server code (route handlers, server actions, server components).
